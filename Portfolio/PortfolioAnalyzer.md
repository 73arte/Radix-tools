...
- Transparent limitations: data freshness, oracle latency,
  minor temporal discrepancies acknowledged  
- Community aggregation: consumes existing public endpoints &
  on-chain data (no duplication of analytics work)
...
- DeFi derivatives  
  ...
  - Pool receipt tokens  
- Yield-generating positions (swap fees, lending interest,
  incentives — treated by economic origin)  
- NFTs  
...
staking exposure is materialized exclusively through tokenized derivatives 
(mainly LSU fungible tokens). Unstaking claims are temporary NFTs.
...
- **Automated historical tracking**  
  ...
  - Time-series view of portfolio evolution (gains/losses, yield changes,
    position shifts)  
...
You must:  
- Keep the original attribution ("Concept by 73arte")
  visible in your project (README, about page, UI footer)  
- Not use it for purely commercial purposes without agreement  

If your implementation generates revenue 
(premium features, subscriptions, paid exports, ads, sponsorships, 
donations with perks, app-store monetization, etc.), 
you agree to pay **5% royalties** on net revenue to the author (73arte), 
paid in XRD to a dedicated royalties wallet address 
(provided privately upon serious implementation discussions).

This is a moral & community convention — not enforceable by smart contract, 
but expected in the Radix open ecosystem.

### Challenges for Developers
1. Resolve real underlying XRD value of any LSULP / composable derivative in < 2s
   using public Radix API + pool state queries.  
3. Aggregate multi-account positions with dynamic filtering without
